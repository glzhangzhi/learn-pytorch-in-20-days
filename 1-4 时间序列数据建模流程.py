...
df.plot(x='data', y=['confirmed_num', 'cured_num', 'dead_num'], figsize=(10, 6))
plt.xticks(rotation=60)
# 看各个数据的趋势

dfdata = df.set_index('date')
dfdiff = dfdata.diff(periods=1).dropna()
dfdiff = dfdiff.reset_index('data')

dfdiff.plot(x='data', y=['confirmed_num', 'cured_num', 'dead_num'], figsize=(10, 6))
plt.xticks(rotation=60)
dfdiff = dfdiff.drop('data'. axis=1).astype('float32')
...
class Block(nn.Module):
    ...
    def forward(self, x, x_input):
        x_output = torch.max((1+x)*x_input[:,-1,:], torch.tensor(0.0))
        return x_output
...
# 这里使用类形式的训练循环
# 仿照Keras定义一个高阶的模型接口Model，实现fit，validate，predict，summary方法，相当于用户自定义高阶API

# 注：循环神经网络调试较为困难，需要设置多个不同的学习率多次尝试，以取得较号的效果
...
# 四。评估模型
# 一般情况下要设置验证集或者测试集，由于此例数据较少，我们仅仅可视化损失函数在训练集上的迭代情况
